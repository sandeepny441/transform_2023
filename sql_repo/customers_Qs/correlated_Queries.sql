...
select * from customers c1 
where 10 < (select count(*) from customers c2 where c1.state = c2.state)

--4-- Write a correlated subquery to find customers who have the highest creditLimit in their country.
select customerName from customers c1 
where creditLimit = (select max(creditLimit) from customers c2 
where c1.country = c2.country)
...
select customerName from customers c1 
where salesRepEmployeeNumber = (select salesRepEmployeeNumber from customers c2 
where 
c1.salesRepEmployeeNumber = c2.salesRepEmployeeNumber
AND
